...
- пока не скинута следующая ссылка - происходит обсуждение предыдущего видео
- добавить периодически в сообщения ссылку на google форму для оставления жалобы или наоборот пожеланий для доработок
- прихранивать субтитры в бд
- надо доработать system prompt, чтобы вот такие фразы не проскакивали:
  ```
  Я вижу, что вы хотите найти информацию о Ченнинге Татуме. К сожалению, у меня нет доступа к инструментам для поиска информации в интернете или базах данных.
  ```
  также доработать system prompt, чтобы бот не общался на другие темы, только то, что касается содержимого видео
  также надо учесть, что люди могут злоупотреблять ботом и присылать сразу несколько ссылок на пересказ (это надо учесть при оплате) 
- возникают ошибки взаимодейсвтия с тг
- переделать использование YoutubeLinkHelper, пусть у llm будет tool на проверку ссылки, но она не должна присылаться на входе сразу (наверное)
- вынести подписку в стартер, чтобы сделать ее универсальной
- бот долго отвечает и не всегда понятно, ждать ли ответа
- модель не должна рассказывать кто она
